...
class MeshOptimizer:
    ...
    def preprocess_point_cloud(self, voxel_size: float = 0.005, target_reduction=0.9, voxel_size_start=0.01,
        ...
        try:
            ...
            print(f"点云预处理出错: {e}")
    # def preprocess_point_cloud(self, voxel_size: float = 0.005) -> None:
    #     """预处理点云数据"""
    #     if self.pcd is None:
    #         return
    #
    ...
    #         print(f"点云体素降采样完成，点数从 {original_points} 减少到 {len(self.pcd.points)}")
    #
    #     # 估计法线
    #     if not self.pcd.has_normals():
    #         print("估计点云法线...")
    ...
    def reconstruct_delaunay_vertical(self,
                                      ...
                                      knn: int = 30) -> bool:
        """
        Using Delaunay triangulation (Fixed version) in areas with dense and vertical point clouds
        """
        ...
        #make sure the point cloud has the normal vector
        if not self.pcd.has_normals():
            print("估计点云法线...")
            self.pcd.estimate_normals(
        ...
        # 1. Multithreaded calculation of point cloud density
        ...
        try:
            ...
            # check whether the triangualtion result is valid
    ...
    #
    #     try:
    #         # 尝试使用trimesh进行高级修复（需确保已安装trimesh）
    #         import trimesh
    #         from trimesh.repair import fix_non_manifold_edges, fill_holes, remove_degenerate_faces
    #
    #         # 转换为trimesh格式
    #         mesh_trimesh = trimesh.Trimesh(
    #             vertices=np.asarray(self.mesh.vertices),
    #             faces=np.asarray(self.mesh.triangles),
    #             process=False  # 禁止自动处理，避免意外修改
    #         )
    #
    #         # 核心修复步骤（按优先级排序）
    #         mesh_trimesh = fix_non_manifold_edges(mesh_trimesh)  # 修复非流形边
    #         mesh_trimesh = fill_holes(mesh_trimesh, max_hole_size=1e6)  # 填充大孔洞
    #         mesh_trimesh = remove_degenerate_faces(mesh_trimesh)  # 移除退化面
    #         mesh_trimesh = trimesh.smoothing.filter_laplacian(mesh_trimesh, iterations=2)  # 轻度平滑
    #
    #         # 转换回Open3D格式
    #         self.mesh.vertices = o3d.utility.Vector3dVector(mesh_trimesh.vertices)
    #         self.mesh.triangles = o3d.utility.Vector3iVector(mesh_trimesh.faces)
    #         self.mesh.compute_vertex_normals()  # 重新计算法线
    #
    #         print(f"使用trimesh修复完成，耗时: {time.time() - start_time:.2f}秒")
    #
    #     except ImportError:
    #         print("错误：未安装trimesh，无法使用高级修复功能")
    #         return
    #     except Exception as e:
    #         print(f"trimesh修复失败: {e}，尝试Open3D基础修复")
    #         # Open3D基础修复流程
    #         self.mesh = self.mesh.remove_unreferenced_vertices()  # 移除孤立顶点
    #         self.mesh = self.mesh.remove_degenerate_triangles()  # 移除退化三角形
    #         self.mesh = self.mesh.fill_holes()  # 填充小孔洞
    #         self.mesh = self.mesh.remove_non_manifold_edges()  # 移除非流形边
    #         self.mesh.compute_vertex_normals()
    #         print(f"Open3D基础修复完成，耗时: {time.time() - start_time:.2f}秒")
    def mesh_repair(self) -> None:
        """修复网格（最终健壮版本）"""
        if self.mesh is None or self.mesh.is_empty():
            print("警告：网格为空或未生成，无法修复")
        ...
        # 尝试使用 trimesh 修复（带防御性编程）
        try:
            import trimesh
            from trimesh import repair

            # 创建 trimesh 网格（增加异常处理）
            mesh_trimesh = trimesh.Trimesh(
                vertices=vertices,
                faces=triangles,
                process=False
            ...
            # 验证 trimesh 网格有效性
            if mesh_trimesh.is_empty or len(mesh_trimesh.faces) == 0:
                raise ValueError("trimesh 网格为空或无效")

            # 执行修复步骤（分阶段处理，便于调试）
            mesh_trimesh = repair.fix_inversion(mesh_trimesh)

            # 填充孔洞（去掉 max_hole_size 参数）
            mesh_trimesh = repair.fill_holes(mesh_trimesh)

            # 修复非流形边
            mesh_trimesh = repair.fix_non_manifold(mesh_trimesh)

            # 移除退化面
            mesh_trimesh = repair.remove_degenerate_faces(mesh_trimesh)

            # 验证修复结果
            if mesh_trimesh.is_empty or len(mesh_trimesh.faces) < len(triangles) * 0.1:
                raise ValueError("修复后网格质量严重下降")
            ...
            print(f"使用 trimesh 修复完成，面数: {len(self.mesh.triangles)}，耗时: {time.time() - start_time:.2f}秒")
            return

        except Exception as e:
            print(f"trimesh 修复失败: {e}，回退到 Open3D 基础修复")
    ...
    def mesh_simplification(self, target_faces: int = 100000) -> None:
        ...
        try:
            ...
            if current_faces > 2 * target_faces:
                # 先简化50%，再达到目标面数（减少单次计算压力）
                self.mesh = self.mesh.simplify_quadric_decimation(target_number_of_triangles=int(current_faces * 0.5))
